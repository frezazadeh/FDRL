...
class BSEnv(gym.Env):
    def __init__(self, base_station=None, monitoring_time_window=None, configfile=None, log_level=None):
        ...
        OBS_SPACE_DIM = 2
        high = np.array([24, 24],dtype=np.float32,)
        ...
        self.observation = []
        self.observation_space = spaces.Box(np.array([-24,0], dtype=np.float32), high, dtype=np.float32)
        # Farhad compose file     self.observation_space = spaces.Box(np.array([-22,0], dtype=np.float32), high, dtype=np.float32)
    ...
    def step(self, decisions):
        ...
        self.prb_allocation = [z for z in self.prb_allocation]
        print("prb---------------------------", self.PRBs_set)
        '''
        if self.prb_allocation[0] == 0:
            self.prb_allocation[0] =1
        if self.prb_allocation[1] == 0:
            self.prb_allocation[0] =1
        ...
        self.dl_capacity = self.prb_allocation
        ...
        traffic_sav = self.dl_bitrate[0] 
        prb_sav = self.prb_allocation [0]
        ...
        served_traffic = np.zeros((len(self.bs.Allocated_slices)))
        allocation_gap = np.zeros((len(self.bs.Allocated_slices)))
        allocation_gap_out = []
        for i in range(len(self.bs.Allocated_slices)):
            ...
            allocation_gap[i] = self.dl_bitrate[i] - self.dl_capacity[i]
            
            if allocation_gap[i]<0:
                served_traffic[i] = self.dl_bitrate[i]
            else:
                served_traffic[i] = self.dl_bitrate[i] - allocation_gap[i]
            #allocation_gap = self.prb_allocation[i] - self.dl_bitrate[i]
            ...
            #if abs(allocation_gap) < 2:
                #cost_per_agent[i] = 0
            if abs(allocation_gap[i]) < 3:
                cost_per_agent[i]= +1
            else:
                cost_per_agent[i] = -1
            
            print("allocation_gap-----------------------------------------------------", "Agent", i ,  allocation_gap)
            '''
            ...
            allocation_gap_out.append(allocation_gap[i])
        ...
        #self.state = [np.array([np.round(self.dl_capacity[i]/25, 5),
                                #np.round(self.dl_bitrate[i]/30, 5),
                                #self.prb_allocation[i]/24]).reshape(1, self.observation_space.shape[0]) for i in range(len(self.bs.Allocated_slices))]
        self.state = [np.array([round(allocation_gap[i], 0), round(served_traffic[i], 0)]).reshape(1, self.observation_space.shape[0]) for i in range(len(self.bs.Allocated_slices))]
        '''
        print("------------------------------------------------------------------")
        ...
        ''' 
        print("Acccccction---------------------------------------------------",self.prb_allocation)                       
        ...
        return self.state, cost_per_agent, False, dict(drop_t=np.round(self.norm_dropped_traffic, 3),
                                                       ...
                                                       PRB_ALLOCATED=prb_sav,
                                                       traf_tracing=np.round(self.avg_slice_traffic_request, 3),
                                                       traffic_sav_info = traffic_sav,
                                                       gap_sav_info = -allocation_gap[0])
    ...
    def reset(self):
        ...
        self.prb_allocation = []
        self.penalty = 0
        self.state = [ np.zeros((1,self.observation_space.shape[0])) for _ in range(len(self.bs.Allocated_slices))]
        self.state = [z+0.08 for z in self.state]
