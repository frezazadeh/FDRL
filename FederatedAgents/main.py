...
def inter_slice_federation_dueling(sdA, sdB):
...
def compute_agent_data_size(multi_agents, drl_class_name):
    ...
    for ma_ind, ma in enumerate(multi_agents):
        for a in ma.agents:
            ...
            elif drl_class_name == 'Dueling':
                 for par in a.onlineQNetwork.parameters():
                     weight[ma_ind] = weight[ma_ind] + np.array(par.tolist()).size    #todo: fix data measure unit (now it's Bytes)
...
def main():
    ...
    drl_class_name = config.get('DRL', '_class_ML')
    ...
    for j in range(int(config.get('DRL', 'n_episodes'))):
        ...
        for i in range(len(base_stations)):
            ...
            for a_ind, a in enumerate(agents[i].agents):
                filename_to_save = '../data/BS_{}_target_net_agent_{}_{}.h5'.format(base_stations[i].ID, agents[i].slices_id[a_ind], drl_class_name)
                if veNB_config[i]['federation'][a_ind]:
                    for tentative in range(2):
                        try:
                            if drl_class_name == 'DDQN' or drl_class_name == 'DQN':
                                a.train_network.save(filename_to_save)
                            elif drl_class_name == 'Dueling':
                                torch.save(a.onlineQNetwork, filename_to_save)
                            break
                        except:
                            sleep(0.5) # wait while the file is being read by the other agents

                    filename_to_load = ['../data/BS_{}_target_net_agent_{}_{}.h5'.format(vbs_id, agents[i].slices_id[a_ind], config.get('DRL', '_class_ML')) for vbs_id in other_vbs]
                            ...
                            for tentative in range(2):
                                try:
                                    if drl_class_name == 'DDQN' or drl_class_name == 'DQN':
                                        ...
                                        a.update_agent_network(inter_slice_federation(a.train_network, model_loaded))  #todo: list of agents
                                    elif drl_class_name == 'Dueling':
                                        model_loaded = torch.load(filename)
                                        a.update_agent_network(inter_slice_federation_dueling(a.onlineQNetwork.state_dict(),model_loaded.state_dict()))
                                    break
                                except:
